...
$password_acc = htmlspecialchars($_POST['password']);
...
if (mysqli_num_rows($result) > 0) {
    ...
    echo "<p class='bad'align='center'>Account already exist! Choose another email</p>";    
    }
} else {

$sql = "INSERT INTO ACCOUNTS (email, password)
VALUES ('$email', '$password_acc')";

if (mysqli_query($conn, $sql)) {
    echo "<p class='good'align='center'>Wait to create the account...</p>";
	echo'<script src="index7.js"></script>';
} else {
    echo "<p class='bad'align='center'>Error: " . $sql . "<br>" . mysqli_error($conn)."</p>";
}
}
mysqli_close($conn);
