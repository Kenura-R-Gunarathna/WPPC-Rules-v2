...
$table_of_search_results_id_for_delete_5678_67 = htmlspecialchars($_POST['table_of_search_results_id_for_delete']);
...
$sql = "DELETE FROM OUTBOX WHERE `id`='$table_of_search_results_id_for_delete_5678_67' AND `delete`='deleted'";

if (mysqli_query($conn, $sql)) {
    echo '<script>$( document ).ready(function() {alert("Massage deleted permanently");});</script>';
} else {
    echo '<script>$( document ).ready(function() {alert("Error deleting massage permanently: ' . mysqli_error($conn).'");});</script>';
}
